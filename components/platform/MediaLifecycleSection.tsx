...
export default function MediaLifecycleSection() {
  return (
    <section className="w-full bg-slate-100 py-12 md:py-16 lg:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl space-y-4">
          ...
          <h2 className="font-['Space_Grotesk'] text-3xl font-light text-zinc-900 sm:text-4xl md:text-5xl">
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-3 sm:grid sm:grid-cols-2 md:grid-cols-4 lg:flex lg:flex-row lg:items-stretch lg:gap-2">
          {STAGES.map((stage, index) => (
            <div key={stage.step} className="flex w-full items-center lg:w-auto lg:flex-1">
              <div className="flex min-h-[140px] w-full flex-col justify-between rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md">
                <h3 className="font-['Space_Grotesk'] text-sm font-medium text-zinc-900">{stage.step}</h3>
                <p className="mt-2 font-['Space_Grotesk'] text-xs leading-5 text-gray-500">{stage.description}</p>
              </div>
