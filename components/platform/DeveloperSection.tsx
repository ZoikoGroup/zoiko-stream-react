    ...
    <section className="w-full bg-zinc-950 px-4 py-16 md:px-12 lg:px-20 lg:py-28">
      <div className="mx-auto flex w-full max-w-7xl flex-col items-start justify-between gap-12 lg:flex-row lg:gap-16">
        <div className="flex w-full flex-1 flex-col items-start gap-6">
          ...
          <h2 className="font-['Space_Grotesk'] text-3xl font-light leading-tight text-slate-100 sm:text-4xl md:text-5xl md:leading-[1.15]">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2 py-2">
            {CAPABILITIES.map((item) => (
              <span key={item} className="font-['Space_Grotesk'] text-sm font-normal text-slate-400">
                {item}
              </span>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2.5 py-2">
            {RESOURCES.map((resource) => (
              <Link
                key={resource}
                href="#"
                className="inline-flex h-9 items-center justify-center rounded-full border border-gray-800 bg-zinc-900/50 px-3.5 font-['IBM_Plex_Mono'] text-xs text-slate-400 transition-colors hover:border-gray-700 hover:text-white"
              >
                {resource}
              </Link>
            ))}
          ...
          <div className="pt-2">
            <Link
              href="/signup"
              className="inline-flex min-h-[48px] min-w-[192px] items-center justify-center rounded-[10px] bg-gradient-to-r from-teal-400 to-blue-500 px-6 py-3.5 font-['Space_Grotesk'] text-base font-normal text-slate-950 transition-opacity hover:opacity-95"
            >
              Start building
            </Link>
          </div>
        ...
        <div className="flex w-full flex-1 flex-col items-start gap-4">
          <div className="w-full overflow-hidden rounded-2xl border border-gray-800 bg-neutral-900 shadow-2xl">
            <div className="border-b border-gray-800 bg-neutral-900/80 px-4 pt-3">
              <div className="inline-block border-b-2 border-teal-400 pb-2">
                <span className="font-['IBM_Plex_Mono'] text-xs text-white">Sample path</span>
              </div>
            </div>

            <div className="overflow-x-auto p-5 font-['IBM_Plex_Mono'] text-xs leading-6">
          ...
          <div className="flex w-full flex-wrap gap-2 pt-1">
            {WORKFLOW_STEPS.map((step, index) => (
              <div
                key={step}
                className="inline-flex items-center gap-2 rounded-full border border-gray-800 bg-gray-900 px-3 py-1.5 font-['IBM_Plex_Mono'] text-xs text-slate-400"
              >
                <span>{step}</span>
          ...
          <p className="mt-1 font-['IBM_Plex_Mono'] text-xs leading-relaxed text-slate-500">
