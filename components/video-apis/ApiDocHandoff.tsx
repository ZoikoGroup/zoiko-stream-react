...
export default function ApiDocHandoff() {
  ...
  return (
    <section 
      id="api-doc-handoff"
      className="relative w-full py-16 sm:py-24 bg-white dark:bg-zinc-955 text-zinc-900 dark:text-white border-b border-gray-200 dark:border-zinc-900 transition-colors duration-250 font-spaceGrotesk"
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col gap-12 text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full font-spaceGrotesk">
          {cards.map((c, idx) => (
            <div 
              key={idx}
              className="bg-slate-50 dark:bg-zinc-900 rounded-xl border border-gray-250 dark:border-zinc-800 flex flex-col overflow-hidden hover:-translate-y-1 hover:shadow-md transition-all duration-300 min-h-[220px]"
            >
              {/* Thumbnail visual */}
              ...
              {/* Title & icon */}
              <div className="p-4 flex justify-between items-center w-full bg-white dark:bg-zinc-900">
