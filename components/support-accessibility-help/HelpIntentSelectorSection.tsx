    ...
    },
    {
      ...
      icon: (
        <div className="relative w-8 h-8 flex items-center justify-center">
          <PlayCircle className="w-8 h-8 text-[#4f5e74]" strokeWidth={1.5} />
          <Lock className="w-4 h-4 text-[#4f5e74] absolute bg-[#f8fafc] rounded-full p-[2px]" strokeWidth={2} />
        </div>
      ),
    },
    {
      ...
      <div className="flex flex-col gap-[48px] relative z-10 w-full max-w-[1216px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[24px] w-full">
          {cards.map((card, idx) => (
            ...
            >
              <div className="flex items-center justify-center w-[48px] h-[48px] rounded-[12px] bg-white border border-[#e2e8f0] shadow-sm group-hover:border-[#4a8cfb] transition-colors">
                {card.icon}
              </div>
