...
export default function WeddingTrustDoctrine() {
  ...
  return (
    <section className="relative w-full py-16 sm:py-20 bg-white dark:bg-zinc-955 text-zinc-905 dark:text-white border-b border-slate-200 dark:border-zinc-900 transition-colors duration-250">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col gap-10 text-left">
        
        {/* Title Block */}
        <div className="flex flex-col gap-3">
          <h2 className="text-zinc-950 dark:text-white text-2xl font-bold font-spaceGrotesk tracking-tight">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
          {cards.map((c, idx) => (
            <div 
              key={idx}
              className="p-5 bg-slate-50 dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 flex flex-col justify-start items-start gap-2 hover:-translate-y-0.5 transition-all shadow-sm"
            >
              ...
              <p className="text-slate-600 dark:text-zinc-400 text-xs sm:text-sm font-normal font-spaceGrotesk leading-relaxed text-left pl-4">
                {c.desc}
              </p>
            </div>
          ))}
