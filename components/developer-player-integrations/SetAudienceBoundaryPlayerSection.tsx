...
const checklist = [
  'Explicit opt-in consent for interactive telemetry recording.',
  'Audited region mapping rules to satisfy GDPR & CCPA storage limits.',
  'Automatic anonymization of moderated public audience chat files.',
  'Encrypted TLS 1.3 tunnels on all stream playback paths.',
];

export function SetAudienceBoundaryPlayerSection() {
  return (
          ...
          <div className="lg:col-span-7 p-8 bg-slate-50 rounded-2xl border border-gray-200 flex flex-col gap-6 shadow-xs">
            ...
            <div className="flex flex-col gap-4 text-xs">
              <div className="flex flex-col gap-1.5">
                <span className="text-slate-600 font-bold uppercase">Accountable Privacy Owner</span>
                <div className="p-3.5 bg-white rounded-lg border border-gray-200 text-slate-950 text-sm">
                  Trust &amp; Compliance Office
                </div>
              </div>

              <div className="flex flex-col gap-1.5">
                <span className="text-slate-600 font-bold uppercase">Access Control Start / End</span>
                <div className="p-3.5 bg-white rounded-lg border border-gray-200 text-slate-950 text-sm">
                  March 24, 2026 09:30 AM — 01:30 PM EST
                </div>
              </div>

              <div className="flex flex-col gap-1.5">
                <span className="text-slate-600 font-bold uppercase">Attendee Consent Form Schema</span>
                <div className="p-3.5 bg-white rounded-lg border border-gray-200 text-slate-950 text-sm">
                  Form ID: CONSENT-v4-REQD
                </div>
              </div>

              <div className="flex flex-col gap-1.5">
                <span className="text-slate-600 font-bold uppercase">Mandatory Recording Disclosure</span>
                <div className="p-3.5 bg-white rounded-lg border border-gray-200 text-slate-950 text-sm">
                  Continuous Stream HUD + Pop-up Modal
                </div>
              </div>
            </div>
          </div>

          {/* Right Column: Privacy & Compliance Checklist */}
