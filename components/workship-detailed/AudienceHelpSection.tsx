...
export default function AudienceHelpSection() {
  const cards = [
    ...
    },
    {
      ...
      status: 'Pending',
      statusColor: 'bg-slate-500/10 text-slate-500 dark:text-slate-400 border-none'
    },
    {
      title: 'Unsure / assistance needed',
      desc: 'Set up the basic intent and review configuration with an event advisor.',
      status: 'Draft',
      statusColor: 'bg-slate-500/10 text-slate-550 dark:text-slate-400 border-none'
    }
  ];
    ...
    <section className="w-full py-16 sm:py-24 bg-white dark:bg-gray-900 text-zinc-900 dark:text-white transition-colors duration-200 border-t border-slate-200 dark:border-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
        ...
        <div className="lg:col-span-7 flex flex-col justify-start items-start gap-8">
          ...
          <div className="w-full flex flex-col gap-4">
            {cards.map((card, idx) => (
              <div 
                key={idx}
                className="p-5 bg-slate-50 dark:bg-zinc-955 rounded-xl border border-slate-205 dark:border-gray-800 flex items-center justify-between gap-4 transition-shadow hover:shadow-sm"
              >
                <div className="flex items-start gap-4">
                  <div className="w-3 h-3 bg-teal-400 rounded-full mt-2 shrink-0" />
                  <div className="flex flex-col gap-1 text-left">
                    <h4 className="text-zinc-900 dark:text-white text-lg font-bold leading-tight font-sans">
                      {card.title}
                    </h4>
                    <p className="text-gray-500 dark:text-slate-400 text-sm leading-relaxed max-w-xl">
                      {card.desc}
                    </p>
                  </div>
                </div>
        ...
        <div className="lg:col-span-5 flex justify-center items-center">
          <img 
            className="w-full max-w-[526px] aspect-[526/765] object-cover rounded-3xl shadow-xl border border-slate-200 dark:border-gray-700" 
