  ...
  return (
    <section className="relative w-full py-16 sm:py-24 bg-white text-zinc-900 transition-colors duration-200 border-t border-slate-100">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col gap-12 text-left">
        ...
        <div className="p-6 sm:p-8 bg-slate-50 rounded-2xl flex flex-col justify-start items-start gap-6 shadow-sm backdrop-blur-sm">
          ...
          <div className="w-full overflow-x-auto">
            <div className="min-w-[800px] flex flex-col gap-2">
              {/* Header row */}
              <div className="flex items-center gap-4 px-4 py-3 bg-slate-200/50 rounded-xl text-gray-550 text-xs font-bold uppercase tracking-wider">
              ...
              {ledgerRows.map((row, idx) => (
                ...
                >
                  <div className="w-1/4 text-zinc-900 text-sm font-bold font-sans">{row.layer}</div>
                  <div className="w-1/3 text-gray-500 text-sm font-sans">{row.metadata}</div>
                  <div className="w-1/4 text-gray-505 text-sm font-sans">{row.sync}</div>
        ...
        {/* Status Engine */}
        <div className="p-6 bg-slate-50 rounded-2xl flex flex-col justify-start items-start gap-4 shadow-sm border border-slate-100">
              ...
              <span 
                ...
                className={`px-4 py-2 text-xs font-bold rounded-full transition-colors cursor-default ${
                  status.active 
                    ...
                    : 'bg-slate-200 text-gray-500 hover:bg-slate-300/50'
                }`}
              >
                {status.name}
              </span>
