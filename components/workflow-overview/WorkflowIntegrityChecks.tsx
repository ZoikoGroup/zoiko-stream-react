  ...
  const blocks = [
    {
      ...
      badge: { text: 'Ready', style: 'text-emerald-500 bg-emerald-100' }
    },
    {
      num: '02',
    ...
    },
    {
      ...
      badge: { text: 'Needs Review', style: 'text-amber-500 bg-amber-100' }
    },
    {
      num: '04',
    ...
    },
    {
      ...
      badge: { text: 'Blocked', style: 'text-red-500 bg-red-100' }
    },
    {
      num: '08',
      title: 'Authorization',
      desc: 'Awaiting primary event owner sign-off signature.',
      badge: { text: 'Pending Gate', style: 'text-slate-500 bg-slate-200' }
    }
  ];
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col gap-12 text-left">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
          {blocks.map((b, idx) => (
            <div 
              key={idx}
              className="p-5 bg-slate-50 rounded-xl border border-slate-200 flex flex-col justify-start items-start gap-3 hover:-translate-y-0.5 hover:shadow-sm transition-all duration-300 min-h-[170px]"
            >
              <div className="flex justify-between items-center w-full">
                <span className="text-blue-500 text-lg font-bold font-mono">
                  {b.num}
                </span>
              ...
              <div className="flex flex-col gap-1.5 w-full text-left">
                <h3 className="text-zinc-955 text-sm sm:text-base font-bold leading-snug">
                  {b.title}
                </h3>
        ...
        {/* Bottom Alert bar */}
        <div className="p-6 bg-slate-100 rounded-xl border border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 w-full mt-4">
          <div className="flex-1 flex flex-col gap-1 text-left">
            <h4 className="text-zinc-955 text-base font-bold">
          ...
          <button className="px-6 py-3.5 rounded-[10px] outline outline-1 outline-offset-[-1px] outline-slate-600 bg-transparent hover:outline-teal-400 transition-colors text-zinc-955 text-sm font-semibold shrink-0 cursor-pointer">
            Review readiness
          </button>
        </div>
